...
//checkbox
var checkboxFun = function checkboxFun(elem, opts) {
  //默认参数
  var defaults = {
    valElemL: '.place-val',
    turnStatus: false,
  ...
  switch (this.option.style) {
    case 'red':
      this.option.lineColor = '#ff3c00';this.option.bgColor = '#ffd8cc';break;
    default:
      this.option.lineColor = '#099bff';this.option.bgColor = '#ceebff';
  }
...
$.extend(checkboxFun.prototype, {
  ...
  _render: function _render() {
    var speed = 0,
        end = this.option.nowVal,
        _this = this;
    (function rafsAnimate() {
      var raf = window.requestAnimationFrame(rafsAnimate);
      _this.nowval = speed;
      ...
      var av = (end - speed) / end + _this.easing;
      speed += av;
    })();
  },
  roundBorder: function roundBorder() {
    this.ctx.beginPath();
  ...
  roundProcess: function roundProcess() {
  ...
  setVal: function setVal() {
    var v = !!arguments[0] ? this.nowval = arguments[0] : this.option.nowVal;
    this._render();
  }
});
//注册全局
